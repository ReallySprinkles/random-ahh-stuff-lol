export default async () => {
  ...
  // Shuffle aweme_list array in-place
  for (let i = data.aweme_list.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [data.aweme_list[i], data.aweme_list[j]] = [data.aweme_list[j], data.aweme_list[i]];
  }

  // Return the shuffled response
